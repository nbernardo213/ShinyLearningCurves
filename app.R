...
server <- function(input, output) {
  ...
  LegoSetColor=data.frame(Red = c("Car (2 Brick)","Crab (2 Brick)","Windmill (1 Brick)"),
                          Blue = c("House (1 Brick)","Train (1 Brick)","Whale (2 Brick)"),
                          Yellow=c("Lion (2 Brick)","Plane (2 BRick)","Shop (1 Brick)"),
 ...
 output$intro<-renderText({"A learning curve demonstrates how operaters improve the speed at which they can complete a task through training.<br>
                          <br>
 ...
 DF2<-reactive({data.frame(x=indat$data[,1],
                           y1=indat$data[,2],
                           y2=indat$data[,3],
                           y3=indat$data[,4])
 })
 ...
 output$equation3<-renderText({
   ...
   paste(as.character(LegoSetColor[3,input$selected]),
         ": y = ",
         round(coef(nls(y3~a*x^b,data = DF2(),start = list(a=1,b=1)))[1],4),
         "x^(",
...
##### Generate Plots #####
 output$Individual<- renderPlot({
   ...
   eqind<-function(x){(indat$data[1,input$setselect])/(1^((log10(indat$data[16,input$setselect])-log10(indat$data[1,input$setselect]))/(log10(16)-log10(1))))*x^((log10(indat$data[16,input$setselect])-log10(indat$data[1,input$setselect]))/(log10(16)-log10(1)))}
 ...
 output$Combined <-    renderPlot({ 
   if (input$selectCombined == "Data Only"){
     ...
     ggplot(data = datalong(),aes(datalong()[,1],Duration,color=Lego))+
       ...
       geom_line()+
       ggtitle(paste(input$selected," Set"))+
       theme(plot.title = element_text(size=20, hjust = .5))+
       xlab("Trials")+
     ...
     eq1<-function(x){(indat$data[1,2])/(1^((log10(indat$data[16,2])-log10(indat$data[1,2]))/(log10(16)-log10(1))))*x^((log10(indat$data[16,2])-log10(indat$data[1,2]))/(log10(16)-log10(1)))}
     eq2<-function(x){(indat$data[1,3])/(1^((log10(indat$data[16,3])-log10(indat$data[1,3]))/(log10(16)-log10(1))))*x^((log10(indat$data[16,3])-log10(indat$data[1,3]))/(log10(16)-log10(1)))}
     eq3<-function(x){(indat$data[1,4])/(1^((log10(indat$data[16,4])-log10(indat$data[1,4]))/(log10(16)-log10(1))))*x^((log10(indat$data[16,4])-log10(indat$data[1,4]))/(log10(16)-log10(1)))}
